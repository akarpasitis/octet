				...
				if (missile.is_enabled()) {
					...
					for (int j = 0; j != num_invaderers; ++j) {
						...
						if (invaderer.is_enabled() && missile.collides_with(invaderer)) {
							...
							invaderer.translate(20, 0); // this should only occur after animation loop and set the explosion sprite as expl.setrelative (invaderer)
	...
	public:
		...
		void app_init() {
			...
			for (int j = 0; j != num_rows; ++j) {
				for (int i = 0; i != num_cols; ++i) { //copy format for uv map loop
			...
			sprites[first_border_sprite + 2].init(turq, -3, 0, 0.2f, 6);
			sprites[first_border_sprite + 3].init(turq, 3, 0, 0.2f, 6);
